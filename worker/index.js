...
export default {
  async fetch(request, env, ctx) {
    ...
    const moderationVehicleRidesMatch = url.pathname.match(/^\/api\/moderation\/robotaxi-vehicles\/([^/]+)\/rides$/);
    if (moderationVehicleRidesMatch && request.method === 'POST') {
      return withCors(await apiLogVehicleRide(request, env, moderationVehicleRidesMatch[1]), request);
    }
